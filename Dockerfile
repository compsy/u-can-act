# Building stage
#===============
FROM ruby:3.1.2

ARG precompileassets
# set from --build-arg
ARG RAILS_ENV
ARG NODE_ENV
ARG SECRET_KEY_BASE
ARG HOST_URL
ARG HOST_DOMAIN
ARG INFO_EMAIL
ARG FEEDBACK_EMAIL
ARG POSTGRES_DATABASE
ARG MONGO_DATABASE
ARG MESSAGEBIRD_ACCESS_KEY
ARG MESSAGEBIRD_SEND_FROM
ARG PERSON_SALT
ARG STOP_SUBSCRIPTION_SALT
ARG ADMIN_USERNAME
ARG ADMIN_PASSWORD
ARG MAILGUN_API_KEY
ARG MAILGUN_DOMAIN
ARG FROM_EMAIL_ADDRESS
ARG TOKEN_SIGNATURE_ALGORITHM
ARG AUTH0_CLIENT_ID
ARG AUTH0_DOMAIN
ARG AUTH0_CLIENT_SECRET
ARG AUTH0_REDIRECT_URL
ARG AUTH0_AUDIENCE
ARG AUTH0_SIGNING_CERTIFICATE
ARG REDIS_HOST
ARG REDIS_PORT
ARG REDIS_PASSWORD
ARG TEST_PHONE_NUMBERS
ARG SNITCH_KEY
ARG API_KEY
ARG API_SECRET
ARG IP_HASH_SALT
ARG INFO_SITE_URL
ARG PUSH_SUBSCRIPTION_URL
ARG BASE_PLATFORM_URL
ARG SHARED_SECRET
ARG REGISTRATION_URL
ARG IKIA_CALLBACK_URL
ARG IKIA_CALLBACK_CHILD_URL
ARG IKIA_CALLBACK_DIARY_URL

# Needed for Yarn
RUN apt-get update -qq && apt-get install -y build-essential libpq-dev curl software-properties-common && \
  curl -sL https://deb.nodesource.com/setup_16.x | bash - && \
  curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add - && \
  echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list && \
  apt-get update && \
  apt-get install -y nodejs yarn && \
  apt-get remove -y --purge software-properties-common &&\
  rm -rf /var/lib/apt/lists/*

RUN mkdir /app
WORKDIR /app

RUN gem install bundler

COPY package.json *yarn* ./
RUN yarn install --check-files

COPY Gemfile /app/Gemfile
COPY Gemfile.lock /app/Gemfile.lock
RUN bundle config --global frozen 1 \
  && bundle install \
  && rm -rf /usr/local/bundle/bundler/gems/*/.git \
    /usr/local/bundle/cache/

# Install chromedriver
COPY bin/chromedriver_install_docker bin/chromedriver_install_docker
RUN bin/chromedriver_install_docker

COPY . /app
RUN bundle exec rake i18n:js:export && ls /app/public/javascripts/translations.js

RUN bin/potential_asset_precompile.sh $precompileassets

# Run stage
#==========
# We could do multi stage builds by adding this

#<ADD>
#FROM ruby:2.6.3

#RUN mkdir -p /app
#WORKDIR /app

#COPY --from=builder /usr/local/bundle/ /usr/local/bundle/
#COPY --from=builder /app/ /app/
# </ADD>

EXPOSE 3000

ENTRYPOINT ["/app/lib/docker-entrypoint.sh"]
CMD ["rails","server","-b","0.0.0.0","-p","3000"]

