...
class DrawingGenerator < QuestionTypeGenerator
  ...
  def drawing(question)
    ...
    tag.div(safe_join(body), class: 'drawing')
  end
  ...
  def drawing_container(question)
    ...
    tag.div(safe_join(body), class: 'row')
  end
  ...
  def drawing_log(question)
    body = tag.textarea(nil,
                        class: 'drawing-log',
                        readonly: true,
                        id: idify(question[:id]),
                        name: answer_name(idify(question[:id])))
    tag.div(body, class: 'col s12 m6 hideme')
  end

  def drawing_buttons(question)
    body = []
    body << tag.button('Opnieuw',
                       class: 'btn waves-effect waves-light drawing-clear',
                       id: idify(question[:id], 'clear'))
    body << tag.button('Ok', class: 'btn drawing-ok', id: idify(question[:id], 'ok'))
    body = tag.div(safe_join(body), class: 'col s12')
    tag.div(body, class: 'row section')
  end
end
