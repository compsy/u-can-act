...
class ShowHideQuestion
  class << self
    def should_show?
      ...
      return deal_with_type(show_after_hash) if show_after_hash.key?(:type)

      deal_with_dates(show_after_hash)
    end

    private

    def deal_with_dates(show_after_hash)
    ...
    def deal_with_type(show_after_hash)
