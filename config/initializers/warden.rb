...
Warden::JWTAuth.configure do |config|
  ...
  if config.algorithm == 'RS256'
    begin
      config.secret = OpenSSL::PKey::RSA.new(ENV.fetch('WARDEN_JWT_PRIVATE_KEY', '').gsub('\\n', "\n"))
      config.decoding_secret = OpenSSL::PKey::RSA.new(ENV.fetch('WARDEN_JWT_PUBLIC_KEY', '').gsub('\\n', "\n"))
