...
if Rails.env.development?
  ...
  students.each do |student|
    ...
    puts "mentor dagboek: #{Rails.application.routes.url_helpers.root_url}?u=#{person.external_identifier}&q=#{token}"
  end
  ...
  responseobj.update_attributes!(
    ...
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "mentor voormeting: #{Rails.application.routes.url_helpers.root_url}?u=#{person.external_identifier}&q=#{token}"
  responseobj = prot_sub.responses.last # nameting
  responseobj.update_attributes!(
    open_from: 1.minute.ago,
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "mentor nameting: #{Rails.application.routes.url_helpers.root_url}?u=#{person.external_identifier}&q=#{token}"
  ...
  responseobj.update_attributes!(
    ...
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "student voormeting: #{Rails.application.routes.url_helpers.root_url}?u=#{student.external_identifier}&q=#{token}"
  responseobj = student.protocol_subscriptions.first.responses[10]
  responseobj.update_attributes!(
    open_from: 1.minute.ago,
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "student dagboek: #{Rails.application.routes.url_helpers.root_url}?u=#{student.external_identifier}&q=#{token}"
  responseobj = student.protocol_subscriptions.first.responses.last
  responseobj.update_attributes!(
    open_from: 1.minute.ago,
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "student nameting: #{Rails.application.routes.url_helpers.root_url}?u=#{student.external_identifier}&q=#{token}"
  ...
  responseobj.update_attributes!(
    ...
    invited_state: Response::SENT_STATE)
  token = responseobj.initialize_invitation_token!
  puts "student dagboek - Bijna in streak -: #{Rails.application.routes.url_helpers.root_url}?u=#{student.external_identifier}&q=#{token}"
