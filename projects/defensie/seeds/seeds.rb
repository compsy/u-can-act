...
if Person.all.select{|person| person.auth_user.blank?}.count == 0 && (Rails.env.development? || Rails.env.staging?)
  ...
  demo_organization = 'Defensie-organization'
  demo_team = 'Defensie-team'
  solo_role_title = 'Defensie-solo'
  solo_protocol = 'defensie-solo-protocol'
  ...
  puts Team.find_by(name: demo_team).roles.where(group: Person::SOLO).to_a.pretty_inspect
  ...
  puts "Inspire: #{invitation_set.invitation_url(invitation_token.token_plain)}"
