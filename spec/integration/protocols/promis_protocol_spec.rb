...
describe 'PROMIS Protocol', type: :integration do
  let(:protocol) { Protocol.find_by(name: 'promis') }
  let(:questionnaire) { Questionnaire.find_by(name: 'promis') }
  let(:person) { FactoryBot.create(:person) }

  before do
    # Create questionnaire first (required by protocol seed)
    load Rails.root.join('projects/sport-data-valley/seeds/questionnaires/promis.rb')
    # Run the protocol seed file
    load Rails.root.join('projects/sport-data-valley/seeds/protocols/promis.rb')
  end

  describe 'protocol configuration' do
    it 'exists with correct name' do
      expect(protocol).to be_present
      expect(protocol.name).to eq('promis')
    end
    ...
    it 'has push subscription configured' do
      push_subscription = protocol.push_subscriptions.find_by(name: 'base-platform-subscription-promis')
  ...
  describe 'measurement configuration' do
    ...
    it 'has a measurement linked to promis questionnaire' do
  ...
  describe 'protocol subscription' do
    ...
    it 'all responses reference the promis questionnaire' do
      protocol_subscription.responses.each do |response|
