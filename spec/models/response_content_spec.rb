...
describe ResponseContent do
  it 'should have valid default properties' do
    responsecontent = FactoryGirl.build(:response_content)
    expect(responsecontent.valid?).to be_truthy
  end

  describe 'content' do
    it 'should accept nil' do
      responsecontent = FactoryGirl.build(:response_content, content: nil)
      expect(responsecontent.valid?).to be_truthy
    end
    it 'should accept an empty hash' do
      responsecontent = FactoryGirl.build(:response_content, content: {})
      expect(responsecontent.valid?).to be_truthy
    end
    it 'should accept a hash' do
      content_hash = { 'v4' => 'goed', 'v5' => ['brood', 'kaas en ham'], 'v6' => 36.2 }
      responsecontent = FactoryGirl.create(:response_content, content: content_hash)
