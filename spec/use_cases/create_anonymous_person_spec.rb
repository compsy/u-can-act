...
describe CreateAnonymousPerson do
  describe 'with team with roles' do
    ...
    it 'should initialize it with the correct role' do
      described_class.run!(team_name: team.name)
      person = Person.last
      expect(person.role).to eq team.roles.first
    end

    it 'should set the name of the person to a default' do
      described_class.run!(team_name: team.name)
      person = Person.last
  ...
  describe 'with team without roles' do
    ...
    it 'should raise if a team is provided that does not have roles' do
